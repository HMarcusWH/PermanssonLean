...
EXPECTED_PDF = 'e71f0c46499fc9e72fdd5f048e2293e29986b573ecc4cf8e0b297afd455f690c'
EXPECTED_TEX = 'a0f6e7e3809f246ff1357e94947f343d08a91ebe5c90e7c7fee5db4eb7b306e0'
...
for name,needle in [
    ('fig3_finite_gate_label', 'Finite-horizon persistence gate'),
    ('fig3_finite_gate_formula', r'\inf_{y\in B}(K_B^L\mathbf 1_B)(y)\ge 1-\eta'),
    ('fig3_exact_implication', r'implies $(L,0)$-persistence'),
    ('fig3_exact_implication_finite_L', r'for every finite $L$'),
    ('fig3_qsd_certificate', r'\mu_BK_B=\theta_B\mu_B'),
    ('fig3_qsd_guardrail', 'The QSD equation alone does not imply the uniform finite-persistence gate.'),
    ('fig3_caption_title', r'\caption{Persistence implications and QSD certification.'),
]: add(name, needle in tex, needle)
add('fig3_internal_duplicate_title_absent', r'\node[font=\small\bfseries] at (0,2.35) {Persistence implications and QSD certification};' not in tex, '')
add('fig3_horizontal_label_has_white_clearance', 'fill=white,inner sep=1.2pt' in tex, '')
add('fig3_exact_box_raised', r'(exact) at (0,1.55)' in tex, '')
add('fig3_boxes_spread_horizontally', r'(finite) at (-4.15,-1.15)' in tex and r'(qsd) at (4.35,-1.15)' in tex, '')
add('fig3_old_hierarchy_caption_absent', 'Persistence hierarchy in v0.1.7' not in tex, '')
add('fig3_baked_figure_number_absent', 'Figure 3: Persistence implications and QSD certification' not in tex, '')
...
r32=(GATE/'RUN_32_FIGURE_CORRECTION_GATE_REPORT.md').read_text() if (GATE/'RUN_32_FIGURE_CORRECTION_GATE_REPORT.md').exists() else ''
add('run31_preserved', 'PASS - 42/42' in r31, '')
add('run32_preserved', 'PASS - 45/45' in r32, '')
...
for name,needle in [
    ...
    ('pdf_fig3_finite_gate','Finite-horizon persistence gate'),
    ('pdf_fig3_qsd','QSD-certified quasi-regime'),
    ('pdf_fig3_finite_L','for every finite L'),
    ('pdf_fig3_guardrail','The QSD equation alone does not imply the uniform finite-persistence gate.'),
]: add(name, needle in pdftxt, needle)

with tempfile.TemporaryDirectory(prefix='permannson_run33_') as td:
    td=Path(td)
...
result={'run':33,'name':'v0.1.8 final Figure 3 layout-freeze gate','passed':passed,'total':total,'result':'PASS' if passed==total else 'FAIL','checks':checks}
(GATE/'RUN_33_RESULTS.json').write_text(json.dumps(result,indent=2)+'\n')
lines=['# Run 33 - v0.1.8 final Figure 3 layout-freeze gate','',f"**Result: {result['result']} - {passed}/{total} checks passed.**",'']
for c in checks:
    lines.append(f"- {'PASS' if c['ok'] else 'FAIL'} **{c['name']}**" + (f" - {c['detail']}" if c['detail'] else ''))
(GATE/'RUN_33_FINAL_FIGURE3_LAYOUT_GATE_REPORT.md').write_text('\n'.join(lines)+'\n')
